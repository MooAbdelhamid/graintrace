# inside pages/3_Admin.py

import streamlit as st
from lib import api_client
from lib.schemas import Bow


def login():
    st.title("Admin Login")
    username = st.text_input("Username")
    password = st.text_input("Password", type="password")
    if st.button("Login"):
        if username == "admin" and password == "pw":
            st.session_state["admin_logged_in"] = True
            st.rerun()
        else:
            st.error("Invalid credentials")


def admin_page():
    if not st.session_state.get("admin_logged_in", False):
        login()
        return

    st.title("Admin Panel")
    option = st.radio("Select admin action:", ["Enrol Bow", "Delete Bow"])

    if option == "Enrol Bow":
        mod_enrol_bow()
    elif option == "Delete Bow":
        delete_bow()


def mod_enrol_bow():
    """Enrol a new bow. Sends the passport + image to the orchestrator."""

    from datetime import date  # added for min_value of date input field

    st.set_page_config(page_title="Enrol a bow", layout="wide")
    st.title("Enrol a bow")
    st.caption("Fill in the passport, upload one head photo, submit.")

    # ---------------------------------------------------------------------------
    # Passport form
    # ---------------------------------------------------------------------------

    # adjust date input widget appearance to align with text input widgets
    st.markdown(
        """
        <style>
        div[data-baseweb="datepicker"] > div {
            min-height: 38px !important;
        }
        input[type="text"] {
            height: 38px !important;
            font-size: 14px !important;
            padding: 19px 10px !important;
        }
        </style>
        """,
        unsafe_allow_html=True,
    )

    st.subheader("Characteristics")

    col1, col2 = st.columns(2)

    with col1:
        maker = st.text_input("Maker *", placeholder="e.g. Eugène Sartory")
        bow_kind = st.text_input("Kind of bow", placeholder="e.g. Violin bow")
        brand = st.text_input("Brand", placeholder="Punch on the bow, e.g. E. SARTORY")
        school = st.selectbox(
            "School / origin",
            ["", "French", "English", "German", "American", "Italian", "Other"],
        )
        owner = st.text_input("Owner")

    with col2:
        proof_created_by = st.text_input(
            "Proof created by", placeholder="Name and address of the issuer"
        )
        place_of_issue = st.text_input("Place of issue")
        date_of_issue = st.date_input(
            "Date of issue", min_value=date(1800, 1, 1), format="YYYY-MM-DD"
        )  # CHANGED TO ENFORCE DATE FORMAT
        wood_registration_date = st.date_input(
            "Wood registration date", min_value=date(1800, 1, 1), format="YYYY-MM-DD"
        )  # CHANGED TO ENFORCE DATE FORMAT
        import_proof = st.text_input("Import proof")

    st.divider()

    st.subheader("IDs")

    col1, col2 = st.columns(2)

    with col1:
        st.caption("External identifiers (optional)")
        maker_assigned_id = st.text_input("Maker's own catalog number")
        certificate_no = st.text_input("Certificate No.")
        stick_id_no = st.text_input("Stick ID No.")

    with col2:
        st.write("")  # add blank line
        st.write("")

        notes = st.text_area(
            "Notes / specifics",
            placeholder="Condition, repair history, distinguishing marks…",
            height=243,
        )

    st.divider()

    # ---------------------------------------------------------------------------
    # Materials
    # ---------------------------------------------------------------------------

    st.subheader("Materials")
    st.caption("Fill in whichever parts the passport lists — blank fields are ignored.")

    material_keys = [
        "stick",
        "faceplate",
        "mounting",
        "lapping",
        "frog",
        "slide",
        "eye",
        "metal",
        "hair",
    ]

    placeholders = {
        "stick": "e.g. Pernambuco",
        "faceplate": "e.g. Ivory",
        "mounting": "e.g. Silver",
        "lapping": "e.g. Bovine leather",
        "frog": "e.g. Ebony",
        "slide": "e.g. Mother-of-pearl",
        "eye": "e.g. Abalone",
        "metal": "e.g. Nickel-silver",
        "hair": "e.g. Horse hair",
    }

    materials: dict[str, str] = {}
    mcols = st.columns(3)
    for i, key in enumerate(material_keys):
        with mcols[i % 3]:
            v = st.text_input(key.capitalize(), placeholder=placeholders[key])
            if v.strip():
                materials[key] = v.strip()

    st.divider()

    # ---------------------------------------------------------------------------
    # Photo + submit
    # ---------------------------------------------------------------------------

    st.subheader("Photo")
    photo = st.file_uploader(
        "Bow head photo",
        type=["jpg", "jpeg", "png"],
        label_visibility="collapsed",
    )

    if photo is not None:
        st.image(photo, use_container_width=False, width=400)

    st.write("")
    submit = st.button(
        "Register bow",
        type="primary",
        disabled=not (maker and photo),
    )

    if submit:
        try:
            bow = Bow(
                maker=maker.strip(),
                bow_kind=bow_kind.strip() or None,
                brand=brand.strip() or None,
                school=school or None,
                owner=owner.strip() or None,
                maker_assigned_id=maker_assigned_id.strip() or None,
                certificate_no=certificate_no.strip() or None,
                stick_id_no=stick_id_no.strip() or None,
                proof_created_by=proof_created_by.strip() or None,
                place_of_issue=place_of_issue.strip() or None,
                date_of_issue=date_of_issue.isoformat() if date_of_issue else None,
                wood_registration_date=wood_registration_date.isoformat()
                if wood_registration_date
                else None,
                import_proof=import_proof.strip() or None,
                materials=materials,
                notes=notes.strip(),
            )

            with st.spinner("Registering..."):
                result = api_client.register(bow, photo.getvalue(), filename=photo.name)

            st.success(f"Registered: **{result.get('bow_id', '(no id in response)')}**")

            st.json(result)

        except Exception as e:
            st.error(f"Registration failed: {type(e).__name__}: {e}")


def delete_bow():
    """Delete a bow from both databases by system-generated ID. Returns status."""

    st.set_page_config(page_title="Delete a bow", layout="wide")
    st.title("Delete a bow")
    st.caption("Provide the GrainTrace bow ID.")

    bow_id = st.text_input("Bow ID", placeholder="Enter the system-generated bow ID")

    st.write("")
    submit = st.button(
        "Delete bow",
        type="primary",
        disabled=not bow_id.strip(),
    )

    if submit:
        try:
            # bow = Bow(
            #     id=bow_id.strip(),
            # )

            with st.spinner("Deleting..."):
                result = api_client.delete(bow_id)

            st.success(f"Deleted: **{result.get('bow_id', '(no id in response)')}**")

            st.json(result)

        except Exception as e:
            st.error(f"Deleting failed: {type(e).__name__}: {e}")


admin_page()
